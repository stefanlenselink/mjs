...
char* http_get_index()
{
    return strdup(
            ...
            "<strong>Test:</strong>\n"
            "<a href=\"#\" onclick=\"$.ajax({url: '/playlist', success: function(data){$('#getplaylist').html(JSON.stringify(data)); }}); return false;\">get</a>\n"
            ...
            "<a href=\"#\" onclick=\"$.ajax({url: '/playlist/' + $('#getfileinput').val(), success: function(data){$('#getfile').html(JSON.stringify(data)); }}); return false;\">get</a>\n"
            "<pre id=\"getfile\"></pre>\n"
            "</p>\n"
            "\n"
