		...
		case 0:
			...
			if (conf->buffer > 0) {
				...
				if (conf->c_flags & C_MONO){
					if(*conf->snd_system != '\0')
					{
						execlp(conf->mpgpath, "mjs-output", "-m", "-b", buf, "-o", conf->snd_system, "-R", "-", (char *)NULL);
					}else
					{
					...
					if(*conf->snd_system != '\0')
					{
						execlp(conf->mpgpath, "mjs-output", "-o",conf->snd_system,"-b", buf, "-R", "-", (char *)NULL);
					}else
					{
						execlp(conf->mpgpath, "mjs-output", "-a",conf->output,"-b", buf, "-R", "-", (char *)NULL);
					}
				}
				...
				if (conf->c_flags & C_MONO){
					if(*conf->snd_system != '\0')
					{
						execlp(conf->mpgpath, "mjs-output", "-m", "-o", conf->snd_system, "-R", "-", (char *)NULL);
					}else
					{
					...
					if(*conf->snd_system != '\0')
					{
						execlp(conf->mpgpath, "mjs-output", "-o", conf->snd_system, "-R", "-", (char *)NULL);
					}else
					{
